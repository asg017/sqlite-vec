...
void test_rescore_quantize_float_to_int8() {
  printf("Starting %s...\n", __func__);
  // Fixed-range quantizer: maps the assumed input domain [-1, 1] linearly
  // onto [-128, 127] via v = (x + 1) * 127.5 - 128, clamped and truncated.
  int8_t dst[256];

  // Endpoints of the [-1, 1] domain map to the int8 extremes. The low end is
  // exact; the high end may lose 1 LSB to float rounding of the step size.
  {
  ...
  // 0.0 sits mid-domain: (0+1)*127.5 - 128 = -0.5 -> truncates to 0.
  {
    float src[1] = {0.0f};
  ...
  // +/-0.5 map symmetrically around the midpoint.
  {
    float src[2] = {0.5f, -0.5f};
    _test_rescore_quantize_float_to_int8(src, dst, 2);
    assert(dst[0] == 63);   // 1.5*127.5 - 128 = 63.25 -> 63
    assert(dst[1] == -64);  // 0.5*127.5 - 128 = -64.25 -> -64
  }

  // Values outside [-1, 1] are clamped, not wrapped.
  {
    float src[2] = {5.0f, -5.0f};
    _test_rescore_quantize_float_to_int8(src, dst, 2);
    assert(dst[0] == 127);
    assert(dst[1] == -128);
  }

  // All outputs remain within the int8 range.
  {
    float src[4] = {-100.0f, 0.0f, 100.0f, 0.5f};
    _test_rescore_quantize_float_to_int8(src, dst, 4);
    for (int i = 0; i < 4; i++) {
      assert(dst[i] >= -128 && dst[i] <= 127);
    }
    assert(dst[0] == -128);  // clamped low
    assert(dst[2] == 127);   // clamped high
    assert(dst[3] > 0);      // 0.5 is above the midpoint
  }
